...
/// <summary>
/// Тесты GravityLayoutEngine: маршрутизация дуг, ортогональность, обход нод.
/// </summary>
public sealed class GravityLayoutEngineTests
{
	/// <summary>
	/// Дуга из правого порта A в верхний порт B (ноды по диагонали).
	/// Проверяет что internal points идут горизонтально/вертикально и не пересекают ноды.
	/// </summary>
	[Fact]
	...
	/// <summary>
	/// Дуга из правого порта A в левый порт B (ноды рядом по горизонтали).
	/// Проверяет что internal points идут горизонтально слева направо — прямой путь.
	/// </summary>
	[Fact]
	...
	/// <summary>
	/// Дуга из правого порта A в левый порт B (B расположена левее и выше A).
	/// Проверяет что дуга обходит ноду и не пересекает её внутренность.
	/// </summary>
	[Fact]
	...
	/// <summary>
	/// Дуга из правого порта A в верхний порт B (B расположена левее A).
	/// Проверяет что создаётся трёхсегментный маршрут (3 bend) вместо прямого.
	/// </summary>
	[Fact]
	...
	/// Проверяет что дуга не пересекает внутренность ни одной из трёх нод.
	/// </summary>
	[Fact]
	...
	/// <summary>
	/// Три ноды в ряд с включённым отталкиванием дуг от нод (ArcPointNodeRepulsionK=1).
	/// Проверяет что дуга обходит блокирующую ноду и не пересекает ни одну из трёх нод.
	/// </summary>
	[Fact]
